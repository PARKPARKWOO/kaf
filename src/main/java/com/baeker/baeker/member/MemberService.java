...
public class MemberService {
    ...
    //-- Social Join, Login --//
    @Transactional
    public RsData<Member> whenSocialLogin(String provider, String username, String name, String email) {
        Optional<Member> opMember = getMember(username);

        if (opMember.isPresent())
            return RsData.of("S-2", "로그인 되었습니다.", opMember.get());

        return join(provider, username, name, "", "", null, email);
    }
    ...
    private RsData<Member> join(String provider, String username, String name, String about, String password, Integer profileImg, String email) {
        ...
        Member member = Member.createMember(provider, username, name, about, password, profileImg, email);
